...
def run(url,port):
    if islive(url,port)==200:
        ...
        return('<font color="#FF0000">[+]The target Weblogic console address is exposed!<br>[+]The path is: {}<br>[+]Please try weak password blasting!</font> '.format(u))
    else:
        logging.info('[-]Target Weblogic console address not found!')
        return("[-]Target Weblogic console address not found!")
